...
- (id)initWithFrame:(CGRect)frame logic:(KalLogic *)theLogic delegate:(id<KalViewDelegate>)theDelegate
{
  ...
  {
      frame.size.width *= kPad;
      kTileSize.width *= kPad;
      kTileSize.height *= kPad;
  }
...
{
  [[UIImage imageNamed:@"ExpenseDataBG.png"] drawInRect:rect];
}
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
  ...
  if ([hitView isKindOfClass:[KalTileView class]]) {
    ...
    if (tile.belongsToAdjacentMonth) {
      ...
      self.selectedTile = tile;
        //Jinxing Li
        [delegate showExpenseOfDate:tile.date];
    }
  }
  self.highlightedTile = nil;
}
